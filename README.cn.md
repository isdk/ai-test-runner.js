...
- **`$sort`**: **数据排序**。在执行后续断言前，先对实际的数组数据进行排序。通过 `$by` 属性指定排序字段，按数组依次传入则支持多重排序配置；在字段名前加 `-` 前缀表示降序排列。如果省略 `$by`，则执行原生自然排序。排序完成后，算子会将剩余的期望断言规则继续应用在排好序的新数组上。非常适合与 `$first`, `$last` 提取或 `$sequence` 序列断言组合使用（即“流式包装处理”）。
  - 用法示例：`$sort: { $by: "-createdAt", $first: { status: "success" } }`
